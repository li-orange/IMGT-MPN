...
"""
    For GCNs
"""
def train_epoch_sparse(model, optimizer, device, data_loader, epoch, num_tasks, task_type, dataset_idx):
    model.train()
    epoch_loss = 0
    epoch_train_mae = 0
    nb_data = 0
    ...
    for iter, (batch_graphs, batch_targets) in enumerate(data_loader):
        ...
        #冻结其他分类器
        freeze_classifiers(model, dataset_idx)
        ...
        try:
            ...
            batch_scores = model.forward(dataset_idx, batch_graphs, batch_x, batch_e, batch_pos_enc)
        except:
            batch_scores = model.forward(dataset_idx, batch_graphs, batch_x, batch_e)
        #调整维度
        ...
        elif task_type == 'cls' and num_tasks == 1:
    ...
    if task_type == 'cls':
        ...
        print("\ntrain_AUC:",average_auc)
    
    return epoch_loss, epoch_train_mae, optimizer, average_auc

def evaluate_network_sparse(model, device, data_loader, epoch, num_tasks, task_type, dataset_idx, test_val_flag = None):
    model.eval()
    epoch_test_loss = 0
    epoch_test_mae = 0
    nb_data = 0
    ...
    return epoch_test_loss, epoch_test_mae, average_auc
...
# 冻结不相关分类器，只解冻当前任务相关的分类器
def freeze_classifiers(model, dataset_idx):
    # 冻结所有分类器的参数
    ...
    # 共享特征提取器会被默认解冻，如果没有冻结它的话
    # 你不需要显式地冻结它，除非有特别需求

def train_epoch_sparse_new(model, optimizer, device, data_loader, epoch, num_tasks, task_type, dataset_idx):
    model.train()
    epoch_loss = 0
    epoch_train_mae = 0
    nb_data = 0
    ...
    targets = []
    scores = []
    for iter, (batch_graphs, batch_targets, dataset_idx) in enumerate(data_loader):
        batch_graphs = batch_graphs.remove_self_loop()  ###如果不用图卷积就去掉自环
        batch_graphs = batch_graphs.to(device)
        batch_x = batch_graphs.ndata['feat'].to(device)  # num x feat
        batch_e = batch_graphs.edata['feat'].to(device)
...
def evaluate_network_sparse_new(model, device, data_loader, num_tasks, task_type, dataset_idx, test_val_flag = None):
    ...
    with torch.no_grad():
        for batch_idx, (batch_graphs, batch_targets, _) in enumerate(data_loader):
            ...
            #visualize_attention_molecules(attention_score, batch_graphs, save_dir="/home/zsy123/benchmarking-gnns-master/attention_visualization")
