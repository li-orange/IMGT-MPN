...
class MoleculeDatasetDGL(torch.utils.data.Dataset):
    def __init__(self, name='Zinc'):
        ...
        data_root = this_dir / "molecules"
        
        if self.name == 'ZINC-full':
            data_dir = data_root / "zinc_full"
            ...
            data_dir = data_root / "TOX21/tox21_cleaned"
            df = pd.read_csv('%s.csv' % (data_dir))
            ...
            data_dir = data_root / "BBBP/BBBP_valid"
            df = pd.read_csv('%s.csv' % (data_dir))
            graphs = df['smiles'].apply(self.create_graph_from_smiles).tolist()
            ...
            data_dir = data_root / "Sider/sider"
            df = pd.read_csv('%s.csv' % (data_dir))
            graphs = df['smiles'].apply(self.create_graph_from_smiles).tolist()
            ...
            data_dir = data_root / "ClinTox/clintox_valid"
            df = pd.read_csv('%s.csv' % (data_dir))
            graphs = df['smiles'].apply(self.create_graph_from_smiles).tolist()
            ...
            data_dir = data_root / "HIV/HIV_valid"
            df = pd.read_csv('%s.csv' % (data_dir))
            graphs = df['smiles'].apply(self.create_graph_from_smiles).tolist()
            ...
            data_dir = data_root / "FreeSolv/SAMPL"
            df = pd.read_csv('%s.csv' % (data_dir))
            graphs = df['smiles'].apply(self.create_graph_from_smiles).tolist()
            #labels = torch.tensor(df['FDA_APPROVED']).tolist()
            #labels = torch.tensor(df['CT_TOX']).tolist()
            ...
            data_dir = data_root / "Lipophilicity/Lipophilicity"
            df = pd.read_csv('%s.csv' % (data_dir))
            graphs = df['smiles'].apply(self.create_graph_from_smiles).tolist()
            #labels = torch.tensor(df['FDA_APPROVED']).tolist()
            #labels = torch.tensor(df['CT_TOX']).tolist()
...
class MoleculeDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, name, dataset_idx):
        ...
        # 当前文件所在目录（即 data/molecules/）
        this_dir = Path(__file__).resolve().parent
        current_dir = this_dir / "molecules"

        # 数据集路径和名称配置
        # 定义数据集名称和目录
        dataset_paths = {
            'TOX21': ('tox21_8-1-1', current_dir / "TOX21"),
            'BACE': ('bace_8-1-1', current_dir / "BACE"),
            'BBBP': ('BBBP_8-1-1', current_dir / "BBBP"),
            'ClinTox': ('clintox_8-1-1', current_dir / "ClinTox"),
            'HIV': ('HIV_PE', current_dir / "HIV"),
            'Sider': ('sider_8-1-1', current_dir / "Sider"),
            'FreeSolv': ('FreeSolv_8-1-1', current_dir / "FreeSolv"),
            'Lipophilicity': ('Lipophilicity_8-1-1', current_dir / "Lipophilicity"),
            'ESOL': ('ESOL_8-1-1', current_dir / "ESOL"),
            'QM8': ('QM8_8-1-1', current_dir / "QM8")
        }
